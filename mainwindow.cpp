...
{
    ui->setupUi(this);
    statusBar()->showMessage("welcome to NetWorkshark!");
    setWindowIcon(QIcon(":/src/shark"));
    // 设置工具栏
    ...
    connect(ui->actionstart_stop,&QAction::triggered,this,[=]{
        ...
        if(index){
            ...
            if(pointer && res != -1){
                ...
                thread->start();
                ui->actionstart_stop->setIcon(QIcon(":/src/stop.png"));
                countNumber = 0;
                isStart = true;
            }else{ // 无法启动
                index = !index;
            ...
            thread->wait();
            pcap_close(pointer);
            ui->actionstart_stop->setIcon(QIcon(":/src/start.png"));
            isStart = false;
        }
    });
 ...
 * 当 lineEdit 中的文本更改时，它会检查输入信息是否正确
 * 如果更正，则颜色为绿色或红色
*/
 ...
 * 该功能可以响应上下键
 * 当您使用键盘操作时，这个底部的树形小部件也会像您使用鼠标一样显示信息
 */
